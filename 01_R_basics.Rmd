...
+ Subsetting data frames with `dplyr`:
    * `dplyr::filter()` is used to subset rows (cases) of a data frame based on one or multiple conditions (for example, respondents with certain values on one or more variables).
    * `dplyr::select()` is used to subset columns (variables) of a data frame. You can use full variable names or select variables in many other ways. See examples in the `select` [manual page](https://dplyr.tidyverse.org/reference/select.html).
